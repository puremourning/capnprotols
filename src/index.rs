...
impl Index {
    pub fn from_cgr_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let reader =
            serialize::read_message_from_flat_slice(&mut &bytes[..], Default::default())?;
        let cgr = reader.get_root::<schema_capnp::code_generator_request::Reader>()?;
    ...
    pub fn identifiers_at(&self, file: &Path, byte_offset: u32) -> Vec<IdentRef> {
        let Some(fi) = self.lookup_file(file) else { return Vec::new() };
    ...
    pub fn identifier_at(&self, file: &Path, byte_offset: u32) -> Option<IdentRef> {
    ...
    pub fn identifier_in_range(&self, file: &Path, start: u32, end: u32) -> Option<IdentRef> {
    ...
    pub fn remap_file(&mut self, from: &Path, to: &Path) {
        ...
        for (path, fi) in old {
            let new_key = if matches(&path) { to.to_path_buf() } else { path };
    ...
    pub fn import_petname(&self, requesting: &Path, type_id: u64) -> Option<&str> {
        self.lookup_file(requesting)?.imports.get(&type_id).map(String::as_str)
    }
    ...
    pub fn find_node_by_short_name(&self, name: &str, prefer_file: &Path) -> Option<&NodeInfo> {
