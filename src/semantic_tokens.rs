...
fn capture_to_token(name: &str) -> Option<(u32, u32)> {
    ...
    let (ty, modifiers): (T, u32) = match name {
        ...
        "keyword" | "keyword.import" | "keyword.directive" | "keyword.modifier"
        | "keyword.repeat" | "keyword.conditional" | "keyword.operator" | "keyword.function" => {
            (T::KEYWORD, 0)
        }
...
fn highlights_query() -> &'static Query {
    ...
    Q.get_or_init(|| {
        Query::new(tree_sitter_capnp::language(), tree_sitter_capnp::HIGHLIGHTS_QUERY)
...
pub fn full(text: &str) -> Vec<SemanticToken> {
    let Some(tree) = parse(text) else { return Vec::new() };
    ...
    for m in cursor.matches(query, tree.root_node(), text.as_bytes()) {
        for cap in m.captures {
            ...
            let Some((ty, modifiers)) = capture_to_token(name) else { continue };
            let node = cap.node;
